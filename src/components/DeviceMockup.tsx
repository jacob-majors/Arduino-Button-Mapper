  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div style={{
        ...
        <span style={{ fontSize: 9, color: "rgba(56,189,248,0.35)", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase" }}>
          Back Panel
        </span>

        <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
          {Array.from({ length: 4 }).map((_, i) => {
            const port = ports[i] ?? null;
            const portActive = port?.arduinoKey
              ? systemOn && pressedKeys.has(port.arduinoKey)
              : false;
            return (
              <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                ...
                <div style={{
                  ...
                  background: portActive ? "#1a3040" : "#111",
                  border: `2px solid ${portActive ? "#38bdf8" : "#444"}`,
                  boxShadow: portActive ? "0 0 12px rgba(56,189,248,0.6), inset 0 0 6px rgba(56,189,248,0.2)" : "inset 0 2px 4px rgba(0,0,0,0.6)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  transition: "all 0.1s",
                  position: "relative",
                }}>
                  {/* Inner ring — simulates jack barrel */}
                  <div style={{
                    width: 12, height: 12, borderRadius: "50%",
                    background: portActive ? "#0ea5e9" : "#222",
                    border: `1.5px solid ${portActive ? "#7dd3fc" : "#555"}`,
                    transition: "all 0.1s",
                  }} />
                  {/* Tip dot */}
                  <div style={{
                    position: "absolute", width: 4, height: 4, borderRadius: "50%",
                    background: portActive ? "#fff" : "#333",
                  }} />
                </div>
                {/* Port label */}
                <span style={{
                  fontSize: 8, fontWeight: 700, letterSpacing: "0.08em",
                  color: portActive ? "#38bdf8" : "#444",
                  textTransform: "uppercase",
                ...
                {port?.keyDisplay && (
                  <span style={{ fontSize: 7, color: portActive ? "#7dd3fc" : "#333", fontFamily: "monospace" }}>
                    [{port.keyDisplay}]
                  </span>
                )}
              </div>
